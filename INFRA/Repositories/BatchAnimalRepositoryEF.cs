...
namespace INFRA.Repositories
{
    internal class BatchAnimalRepositoryEF : IBatchAnimalRepository
    {
        ...
        public async Task AddAsync(BatchAnimalEntity entity, CancellationToken ct = default)
        {
        ...
        public async Task DeleteAsync(BatchAnimalEntity entity, CancellationToken ct = default)
        {
            await using var db = await _factory.CreateDbContextAsync(ct);
        ...
        public async Task<BatchAnimalEntity?> GetByIdAsync(Guid id, CancellationToken ct = default)
        {
            await using var db = await _factory.CreateDbContextAsync(ct);
            return await db.BatchAnimals.AsNoTracking().SingleOrDefaultAsync(bA => bA.Id == id, ct);
        }

        public async Task<BatchAnimalEntity?> GetByAnimalIdAsync(Guid animalId, CancellationToken ct = default)
        {
            await using var db = await _factory.CreateDbContextAsync(ct);
            return await db.BatchAnimals.AsNoTracking().SingleOrDefaultAsync(bA => bA.AnimalId == animalId, ct);
        }
        public async Task<BatchAnimalEntity?> GetByBatchIdAsync(Guid batchId, CancellationToken ct = default)
        {
            await using var db = await _factory.CreateDbContextAsync(ct);
